...
public class PackageManager {
	...
	public static String getLongPackageContentsString(Session sess,
		...
		if (drugListString.length() > 1) {
			drugListString.delete(1, drugListString.length() - 2);
		}
		...
		if (qtyPackageLabel > 0) {
			...
			pc = new PackageCoverLabel(
					...
					"Issue "
							+ pdi.getPackageIndex()
							+ " / "
							+ (Integer.parseInt(pdi.getPrescriptionDuration()) >= 4 ? (Integer
									.parseInt(pdi.getPrescriptionDuration()) / 4)
									+ " month"
			...
			for (int i = 0; i < pdisForLabels.size(); i = i + 7) {
				...
				esml
						.setIssuesString(pdi.getPackageIndex()
								+ " of a "
								+ (Integer.parseInt(pdi
										.getPrescriptionDuration()) >= 4 ? (Integer
										.parseInt(pdi.getPrescriptionDuration()) / 4)
										+ " month script"
										: pdi.getPrescriptionDuration()
												+ " week script"));
				esml
						.setBoldIssuesString((pdi.getPackageIndex() >= Integer
								.parseInt((pdi.getPrescriptionDuration()
										.split(" "))[0])) ? true : false);
		...
		pdl
				.setIssuesString(pdi.getPackageIndex()
						...
						+ (Integer.parseInt(pdi.getPrescriptionDuration()) >= 4 ? (Integer
								...
								+ " month"
								: pdi.getPrescriptionDuration() + " week"));
		pdl.setBoldIssuesString((pdi.getPackageIndex() >= Integer.parseInt((pdi
				.getPrescriptionDuration().split(" "))[0])) ? true : false);
